...
public sealed class ProgressParser
{
    ...
    public ProgressParser(ILogger? logger = null)
    {
        ...
        _regexHandlers = new Dictionary<Regex, Action<Match>>
        {
            ...
            {
                new Regex(
                    RegexPatterns.DownloadingWebpage,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingWebpage
            },
            {
                new Regex(
                    RegexPatterns.DownloadingJson,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingJson
            },
            {
                new Regex(
                    RegexPatterns.DownloadingTvClientConfig,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingTvClientConfig
            },
            {
                new Regex(
                    RegexPatterns.DownloadingM3u8,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingM3u8
            },
            {
                new Regex(
                    RegexPatterns.DownloadingManifest,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingManifest
            },
            {
                new Regex(
                    RegexPatterns.TotalFragments,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleTotalFragments
            },
            {
                new Regex(
                    RegexPatterns.TestingFormat,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleTestingFormat
            },
            {
                new Regex(
                    RegexPatterns.DownloadingFormat,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingFormat
            },
            {
                new Regex(
                    RegexPatterns.DownloadingThumbnail,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingThumbnail
            },
            {
                new Regex(
                    RegexPatterns.WritingThumbnail,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleWritingThumbnail
            },
            {
                new Regex(
                    RegexPatterns.DownloadDestination,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadDestination
            },
            {
                new Regex(
                    RegexPatterns.ResumeDownload,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
            ...
            {
                new Regex(
                    RegexPatterns.DownloadAlreadyDownloaded,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadAlreadyCompleted
            },
            {
                new Regex(
                    RegexPatterns.DownloadProgressComplete,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadProgressComplete
            },
            {
                new Regex(
                    RegexPatterns.DownloadProgressWithFrag,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadProgressWithFrag
            },
            {
                new Regex(
                    RegexPatterns.DownloadProgress,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
            ...
            {
                new Regex(
                    RegexPatterns.MergingFormats,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleMergingFormats
            },
            {
                new Regex(
                    RegexPatterns.ExtractingMetadata,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleExtractingMetadata
            },
            {
                new Regex(
                    RegexPatterns.SpecificError,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleSpecificError
            },
            {
                new Regex(
                    RegexPatterns.DownloadingSubtitles,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
                HandleDownloadingSubtitles
            },
            {
                new Regex(
                    RegexPatterns.DeleteingOriginalFile,
                    RegexOptions.Compiled | RegexOptions.IgnoreCase
                ),
    ...
    public void Reset()
    {
        ...
        _isMerging = false;
        _logger.Log(LogLevel.Information, "Resetting progress parser.");
    }
    ...
    private void HandleExtractingUrl(Match match)
    {
        ...
        LogAndNotify(LogLevel.Information, $"Extracting URL: {url}");
    }
    ...
    private void HandleDownloadingWebpage(Match match)
    {
        ...
        LogAndNotify(LogLevel.Information, $"Downloading {type} webpage for video ID: {id}");
    }
    ...
    private void HandleDownloadingJson(Match match)
    {
        ...
        LogAndNotify(
            ...
            $"Downloading {type} player API JSON for video ID: {id}"
        );
    }
    ...
    {
        string id = match.Groups["id"].Value;
        LogAndNotify(LogLevel.Information, $"Downloading tv client config for video ID: {id}");
    }
    ...
    {
        string id = match.Groups["id"].Value;
        LogAndNotify(LogLevel.Information, $"Downloading m3u8 information for video ID: {id}");
    }

    private void HandleDownloadingManifest(Match match)
    {
        LogAndNotify(LogLevel.Information, "Downloading manifest");
    }

    private void HandleTotalFragments(Match match)
    {
        string fragments = match.Groups["fragments"].Value;
        LogAndNotify(LogLevel.Information, $"Total fragments: {fragments}");
    }

    private void HandleTestingFormat(Match match)
    {
        string format = match.Groups["format"].Value;
        LogAndNotify(LogLevel.Information, $"Testing format {format}");
    }
    ...
    private void HandleDownloadingFormat(Match match)
    {
        string format = match.Groups["format"].Value;
        string id = match.Groups["id"].Value;
        LogAndNotify(LogLevel.Information, $"Downloading format {format} for video ID: {id}");
    }

    private void HandleDownloadingThumbnail(Match match)
    {
        string number = match.Groups["number"].Value;
        LogAndNotify(LogLevel.Information, $"Downloading video thumbnail {number}");
    }
    ...
    private void HandleWritingThumbnail(Match match)
    {
        ...
        LogAndNotify(LogLevel.Information, $"Writing video thumbnail {number} to: {path}");
    }
    ...
    private void HandleDownloadProgress(Match match)
    {
        ...
        var args = new DownloadProgressEventArgs
        {
            ...
            ETA = etaString,
            Message =
                $"Downloading: {percent:F2}% of {sizeString}, Speed: {speedString}, ETA: {etaString}",
        };
    ...
    private void HandleDownloadProgressWithFrag(Match match)
    {
        ...
        var args = new DownloadProgressEventArgs
        {
            ...
            Message =
                $"Downloading: {percent:F2}% of {sizeString}, Speed: {speedString}, ETA: {etaString}, Fragments: {fragString}",
        };
        LogAndNotify(LogLevel.Information, args.Message);
        OnProgressDownload?.Invoke(this, args);
    }
    ...
    {
        var logType =
            output.Contains("ERROR", StringComparison.OrdinalIgnoreCase) ? LogLevel.Error
            : output.Contains("WARNING", StringComparison.OrdinalIgnoreCase) ? LogLevel.Warning
            : LogLevel.Information;
    ...
    private void HandleMergingFormats(Match match)
    {
        string path = match.Groups["path"].Value;
        _isMerging = true;
        LogAndNotify(LogLevel.Information, $"Merging formats into: {path}");
        _logger.Log(LogLevel.Information, "Set _isMerging to true");
    }
    ...
    private void HandleMergingComplete(Match match)
    {
        ...
        else
        {
            _logger.Log(
                LogLevel.Warning,
                $"Merging complete detected without prior merging start: {match.Value}"
            );
        }
    }
    ...
    {
        string id = match.Groups["id"].Value;
        LogAndNotify(LogLevel.Information, $"Extracting metadata for video ID: {id}");
    }
    ...
    private void HandleDownloadingSubtitles(Match match)
    {
        ...
        LogAndNotify(LogLevel.Information, $"Downloading subtitles for language: {language}");
    }
